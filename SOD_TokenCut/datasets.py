...
class NoGtDataset(Dataset):
    def __init__(self, image_paths, dataset_name, resize=None):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        self.raw_img_size = (img.size[1], img.size[0]) # (H, W)
...
class Dataset:
    def __init__(self, dataset_name, dataset_set, remove_hards):
        ...
        elif dataset_name == "PLSelfSup":
            self.root_path = "datasets/PLSelfSup_sub1000/JPEGImages"
            self.annfile = "datasets/PLSelfSup_sub1000/train.json" # annotation file without groundtruth
            self.pl_anns = get_pl_all_anns(self.annfile)
        else:
    ...
    def load_image(self, im_name, *args):
        ...
        elif "PLSelfSup" in self.dataset_name:
            image = skimage.io.imread(f"./datasets/PLSelfSup_sub1000/JPEGImages/{im_name}")
        else:
    ...
    def get_image_name(self, inp):
        ...
        if "VOC" in self.dataset_name:
            ...
            id = inp[0]["image_id"]
            im_name = self.dataloader.coco.loadImgs(id)[0]["file_name"]
...
def get_pl_all_anns(all_annotations_file):
    # load all annotations
    with open(all_annotations_file, "r") as f:
        anns = json.load(f)
    return anns
